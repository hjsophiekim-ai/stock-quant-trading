...
cd apps\desktop
npm install
# electron / electron-builder 는 devDependencies 에만 있어야 합니다(dependencies 에 두면 일부 환경에서 빌드가 거절됨).
# 저장소 루트에서: npm run desktop:build:win (apps/desktop 으로 위임)
...
산출물: `apps/desktop/dist/Stock Quant Desktop-Setup-0.1.0.exe` (버전은 `apps/desktop/package.json` 의 `version` 과 동일)

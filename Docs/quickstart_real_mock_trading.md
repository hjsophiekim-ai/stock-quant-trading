...
### KIS 모의 API 호출량 (`.env` 권장, EGW00201 완화)

Paper 세션은 **앱 `Settings`(루트 `.env`)** 의 아래 값으로 KIS mock 호출 빈도를 줄입니다. **백엔드 Paper 루프 틱 간격은 `PAPER_TRADING_INTERVAL_SEC`** 입니다 (`RUNTIME_LOOP_INTERVAL_SEC` 와는 별개).

| 환경 변수 | 권장값 | 설명 |
|-----------|--------|------|
| `PAPER_TRADING_INTERVAL_SEC` | `600` | Paper 세션 틱 간격(초) |
| `PAPER_TRADING_SYMBOLS` | `005930,000660` | 쉼표 구분, 기본 2종목 |
| `PAPER_KIS_CHART_LOOKBACK_DAYS` | `60` | 차트·유니버스 룩백 일수 |
| `PAPER_UNIVERSE_CACHE_TTL_SEC` | `300` | 유니버스 TTL(초); `PAPER_KIS_UNIVERSE_CACHE_TTL_SEC` 동일 |
| `PAPER_KOSPI_CACHE_TTL_SEC` | `300` | KOSPI 시계열 TTL; `PAPER_KIS_KOSPI_CACHE_TTL_SEC` 동일 |
| `PAPER_POSITIONS_REFRESH_INTERVAL_SEC` | `900` | 포지션 스냅샷 최소 간격(0이면 매 틱) |
| `PAPER_PORTFOLIO_SYNC_INTERVAL_SEC` | `1800` | `run_portfolio_sync` 최소 간격(0이면 비활성) |
| `RUNTIME_LOOP_INTERVAL_SEC` | `600` | (선택) 전역 런타임 엔진 루프 — Paper와 맞출 때 |

자세한 예시는 루트 **`env.paper.example`** , Render 배포는 **`Docs/deployment_backend.md`** 를 참고하세요.

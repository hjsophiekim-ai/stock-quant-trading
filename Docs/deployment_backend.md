...
- **완화**: `render.yaml`의 `PIP_PREFER_BINARY=1`, 요금제 상향, `requirements/backend.txt` 변경을 자주 하지 않기(Render가 의존성 캐시를 활용하기 쉬움). 구조적 한계는 **단일 프로세스에서 FastAPI가 라우터 전부를 import** 하는 패턴입니다(헬스만 가볍게 하려면 장기적으로 lazy import 설계가 필요).

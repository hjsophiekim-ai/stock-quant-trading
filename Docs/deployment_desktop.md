...
Google Drive 등 동기화 폴더에서는 `node_modules` 설치가 실패할 수 있어, 저장소 루트에서 아래 스크립트는 **임시 폴더(`%TEMP%`)에서 NSIS 빌드**한 뒤 `release/` 에 ZIP을 둡니다.

- 명령: `npm run release:build:zips` (또는 `.\scripts\build-release-zips.ps1`)
- 선택 인자: `-BackendUrl`, `-AppEnv`, 기존 설치 파일만 묶을 때 `-DesktopExePath`, APK 포함 시 `-ApkPath`
- 결과: `release/Stock-Quant-Desktop-Windows-<version>.zip`, `release/Stock-Quant-Android-<version>.zip`
...
- **원클릭 설치** (`oneClick: true`) — 일부 환경에서 다중 사용자/경로 선택 NSIS가 UAC 플러그인 오류를 일으켜 단순화했습니다.
- **바탕화면 바로가기**·**시작 메뉴** 항목 생성
- 설치 완료 후 앱 실행 옵션 (`runAfterFinish: true`)
- 로컬 개발 빌드에서는 **코드 서명 비활성화**(`forceCodeSigning: false`, `signAndEditExecutable: false`)로 Windows 심볼릭 링크 권한 문제를 피합니다. 상용 배포 시에는 서명 정책을 별도로 정하세요.

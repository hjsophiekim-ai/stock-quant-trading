"""Paper 인트라데이 단타 — 1분봉·공격적 모멘텀. 리스크 파라미터는 기본 설정 대비 더 타이트하게 적용."""
    ...
    def generate_signals(self, context: StrategyContext) -> list[StrategySignal]:
        ...
        if not prices.empty:
            for sym in prices["symbol"].unique():
                ...
                mom_ok = last_close >= float(close.iloc[-5]) * 1.0005 if len(close) >= 5 else False
                ema_align = (
                    len(ema3) > 0
                    and len(ema8) > 0
                    and float(ema3.iloc[-1]) > float(ema8.iloc[-1]) > float(ema21.iloc[-1])
                )
                vwap_reclaim = len(vwap) and last_close > float(vwap.iloc[-1]) * 1.0002
                micro_break = last_close > prev_high * 1.0003
                vol_spike = vz is not None and vz > 0.35
                rsi_ok = len(rsi) and float(rsi.iloc[-1]) < 72.0
                body_pct = last_bar_body_pct(sub) or 0.0
                if body_pct > float(cfg.paper_intraday_max_chase_candle_pct) * 0.9:

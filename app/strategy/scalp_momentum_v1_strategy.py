"""Paper 인트라데이 단타 — 비교적 보수적(3분봉 기준). 당일 청산·overnight 금지."""
...
@dataclass
class ScalpMomentumV1Strategy(BaseStrategy):
    """3분봉 모멘텀·VWAP·EMA 골든크로스·OR 돌파. 고변동 국면 신규 진입 제한."""
    ...
    def generate_signals(self, context: StrategyContext) -> list[StrategySignal]:
        ...
        for sym in pos_symbols:
            ...
            if flatten_close:
                ...
                sl = avg * (1.0 - float(cfg.paper_intraday_stop_loss_pct) / 100.0)
                tp = avg * (1.0 + float(cfg.paper_intraday_take_profit_pct) / 100.0)
                trail_pct = float(cfg.paper_intraday_trailing_stop_pct)
                ...
                hold_min = float(cfg.paper_intraday_max_hold_minutes)
                if last_px <= sl:
        ...
        if not prices.empty:
            for sym in prices["symbol"].unique():
                ...
                diag: dict[str, Any] = {"symbol": sym, "entered": False, "blocked_reason": ""}
                if len(sub) < 25:
                ...
                gc = len(ema5) >= 2 and ema5.iloc[-2] <= ema20.iloc[-2] and ema5.iloc[-1] > ema20.iloc[-1]
                vwap_ok = len(vwap) and last_close > float(vwap.iloc[-1])
                rsi_ok = len(rsi) and float(rsi.iloc[-1]) < 68.0
                vol_ok = vz is not None and vz > 0.5
                or_break = or_high is not None and last_close > or_high * 1.001

                if not (rsi_ok and vwap_ok and vol_ok and (gc or or_break)):
